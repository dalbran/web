  ...
  container.innerHTML = `
    <div class="view-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
      ...
      <div style="display:flex; gap:0.5rem;">
        <div class="product-import-menu"><button id="btn-mobile-import-menu" type="button" class="btn btn-outline btn-sm">Importar / Exportar</button><div id="product-import-options" class="product-import-options"><button id="btn-import-json" class="btn btn-outline btn-sm">Importar JSON</button><button id="btn-export-json" class="btn btn-outline btn-sm">Exportar JSON</button></div></div>
        ...
        ${(prod.variacoes || []).map((v, index) => {
          ...
          return `
            <div style="font-size:0.85rem; margin-bottom:0.25rem; background:#f8fafc; padding:0.25rem 0.5rem; border-radius:4px;">
              ...
              Nota fiscal: <span class="quick-edit-price" data-id="${prod.id}" data-vindex="${index}" data-field="precoNotaFiscal" style="cursor:pointer; text-decoration:underline; color:#0284c7;">${typeof formatCurrency === 'function' ? formatCurrency(v.precoNotaFiscal !== undefined ? v.precoNotaFiscal : v.precoVarejo) : 'R$ ' + (v.precoNotaFiscal || v.precoVarejo || 0)}</span>
...
function bindProductsDOMEvents() {
  ...
  const importMenu = document.getElementById('btn-mobile-import-menu');
  if (importMenu) importMenu.onclick = () => document.getElementById('product-import-options').classList.toggle('open');
...
function addVariationRow(data = null) {
  ...
  row.innerHTML = `
    <div style="display:grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap:0.5rem; margin-bottom:0.5rem;">
      ...
      <div>
        <label style="font-size:0.75rem;">Preço Nota Fiscal (R$)</label>
        <input type="number" step="0.01" class="v-nota-fiscal" value="${precoNotaFiscal}" placeholder="Usa varejo se vazio" style="width:100%; padding:0.4rem;">
      </div>
    </div>
    <div>
